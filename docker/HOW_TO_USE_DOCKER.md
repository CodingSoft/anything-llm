# How to use Dockerized CodingSoft

Use the Dockerized version of CodingSoft for a much faster and complete startup of CodingSoft.

### Minimum Requirements

> [!TIP]
> Running CodingSoft on AWS/GCP/Azure?
...
\*CodingSoft by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*CodingSoft by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized CodingSoft!
...
> the docker container using CodingSoft as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull codingsoft/anythingllm
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run CodingSoft in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/codingsoft && \
...
$env:STORAGE_LOCATION="$HOME\Documents\codingsoft"; `
...
services:
  codingsoft:
    image: codingsoft/anythingllm
    container_name: codingsoft
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - codingsoft_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using CodingSoft! All your data and progress will persist between
...
The integrations below are templates or tooling built by the community to make running the docker experience of CodingSoft easier.

### Use the Midori AI Subsystem to Manage CodingSoft

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the CodingSoft docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in CodingSoft.
...
[Ask for help on Discord](https://discord.gg/codingsoft)
